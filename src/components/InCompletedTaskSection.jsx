...
export default function InCompletedTaskSection({ tasks }) {
  ...
  const EditTaskHandler = async (data) => {
    const changedTask = data.inCompletedTask.find(
      ({ id }) => id === editableField
    );
  ...
  return (
    ...
    >
      ...
      >
        {defaultValues.inCompletedTask.map(
          ({ id, completed, userId, todo }, index) => (
            <div
              className="w-full px-2 flex justify-between  text-lg "
              key={id}
            >
              <input
                {...register(`inCompletedTask.${index}.todo`, {
